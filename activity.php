...
function loadActivity(reset=false) {
  ...
  fetch(`/api/activity-log.php?limit=${limit}&offset=${offset}`)
    ...
    .then(res=>{
      ...
      if(!res.success){el.innerHTML='<div class="empty-state"><p>No activity yet.</p></div>';return;}
      const items = res.data.items||[];
      const total = res.data.total||0;
      if(!items.length){el.innerHTML='<div class="empty-state"><p>No activity yet.</p></div>';return;}
